...
from config_web1 import EMAIL_API_TOKEN, G_CLIENT_ID, G_CLIENT_SECRET, admin_mail, telegrambotlink, ouolink, whatsapp_number, whatsapp_channel, telegram_user, telegram_channel, FROM_NAME, FROM_EMAIL, ZEPTO_API_KEY, ZEPTO_API_URL
...
class User(db.Model):
    ...
    iklan_expiry = db.Column(db.DateTime, nullable=True)
...
def daily_broadcast_job():
    with app.app_context():
        print(f"[SCHEDULER] Cek broadcast {datetime.now()}")
        broadcast_lock_notification()  # ini akan kirim otomatis 3 hari sebelum tanggal 14 & akhir bulan

def start_scheduler():
    scheduler = BackgroundScheduler(timezone="Asia/Jakarta")

    # Jalankan setiap hari jam 07:00 WIB (misalnya pagi hari)
    scheduler.add_job(
        daily_broadcast_job,
    ...
    scheduler.start()
    print("[SCHEDULER] Broadcast scheduler aktif setiap jam 07:00 WIB")
...
def upload_to_mega(local_file, remote_folder="/Kocheng_backup/"):
    m = get_mega_client()
    if not m:
...
def restore_mega():
    ...
    remote_file = f"/Kocheng_backup/{safe_filename}"
