...
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER_KOCHENG
...
def delete_pterodactyl_server(user):
    if not user.serverid:
        return False, "User tidak punya panel/serverid."
    ...
    # ✅ Ambil server milik user dari tabel Server
    server = Server.query.filter_by(user_id=user.id).first()
    if not server:
        return False, "Server user tidak ditemukan di database."

    ptero_server_id = server.id  # ID asli Pterodactyl

    try:
        # ==============================
        # 1️⃣ HAPUS SERVER
        # ==============================
        delete_server_url = f"{panel_url}/api/application/servers/{ptero_server_id}"
        response = requests.delete(delete_server_url, headers=headers, timeout=10)

        if response.status_code not in (200, 204):
            return False, f"Gagal menghapus server: {response.status_code} {response.text}"

        # ==============================
        # 2️⃣ CARI USER BERDASARKAN EMAIL
        # ==============================
        search_url = f"{panel_url}/api/application/users?filter[email]={user.email}"
        ...
        if search_response.status_code == 200:
            ...
            if data.get("data"):
                ptero_user_id = data["data"][0]["attributes"]["id"]

                # ==============================
                # 3️⃣ HAPUS USER PTERODACTYL
                # ==============================
                delete_user_url = f"{panel_url}/api/application/users/{ptero_user_id}"
                requests.delete(delete_user_url, headers=headers, timeout=10)
                # Kalau gagal karena masih ada server lain, biarkan saja
...
def get_server_startup(panel_id, server_id):
    panel = PANELS[panel_id]
    headers = {
    ...
    startup = data["container"]["startup_command"]
    env = data["container"]["environment"]
    egg = data["egg"]
    image = data["container"]["image"]

    return startup, env, egg, image
    
def update_startup_nodejs(panel_id, server_id, heap):
    panel = PANELS[panel_id]
    headers = {
        "Authorization": f"Bearer {panel['api_key']}",
        "Content-Type": "application/json",
    ...
    url = f"{panel['url']}/api/application/servers/{server_id}/startup"

    startup, env, egg, image = get_server_startup(panel_id, server_id)

    startup = re.sub(
        r'export NODE_OPTIONS="--max-old-space-size=\d+"',
        f'export NODE_OPTIONS="--max-old-space-size={heap}"',
        startup
    )
    ...
    if not r.ok:
        print("[ERROR] Startup update gagal:", r.status_code, r.text)
        r.raise_for_status()
...
def revert_ram(panel_id, user, server_data, ramasli):
    ...
    heap = get_heap_from_ram(ramasli)
    ...
    try:
        # 1️⃣ Update RAM (WAJIB sukses)
        r = requests.patch(url, headers=headers, json=payload)
        r.raise_for_status()

        # 2️⃣ Update startup (OPSIONAL)
        try:
            update_startup_nodejs(panel_id, server_data.id, heap)
        except Exception as e:
            print(f"[WARNING] Startup tidak diupdate (mungkin bukan NodeJS): {e}")

        # 3️⃣ Restart tetap jalan
        try:
            requests.post(
                f"{panel['url']}/api/application/servers/{server_data.id}/power",
                headers=headers,
                json={"signal": "restart"}
            )
        except Exception as e:
            print(f"[WARNING] Gagal restart server: {e}")

        print(
            f"[INFO] RAM dikembalikan ({ramasli} MB) "
            f"untuk {user.email} (server {server_data.id})"
        )
        return True

    except Exception as e:
        print(
            f"[ERROR] Gagal revert RAM "
            f"{user.email} - {str(e)}"
        )
        return False
...
#------ SISTEM BACKUP ------#
MEGA_API = "https://apimega.kocheng.biz.id"
            ...
            if not user.login_google:
                ...
                if 'photo' in request.files:
                    ...
                    if file and allowed_file(file.filename):
                        ...
                        file.save(file_path)
                        pending['photo_url'] = f'uploads_kocheng/{filename}'

                session['pending_update'] = pending
                session['terakhir_kirim_kode_mail'] = datetime.utcnow().strftime(
                    "%Y-%m-%d %H:%M:%S"
                )

                send_verification_email(new_email, otp_kode)
                flash('OTP sent to new email.')
...
@app.route("/api/monetag/create-session", methods=["POST"])
def create_session():
    data = request.get_json()
    email = data.get("email")
...
def monetag_postback():
    ...
    if not ymid:
        return "Missing YMID", 400

    session = MonetagSession.query.filter_by(ymid=ymid).first()
    if not session:
        return "Invalid YMID", 400
...
@app.route("/api/monetag/check/<ymid>", methods=["GET"])
def check_status(ymid):
    session = MonetagSession.query.filter_by(ymid=ymid).first()
